...
bymydiststat "res${varoi}ch$yrbr$yrer" "D" "stayIIv2" "agebin$yrepr  avgres${varoi}rankage$yrepr stayIIv2_${yrbr}${yrer}"
...
if($yr6F>$yr5F & $yr5F>${yrer}){
	...
	bymydiststat "res${varoi}ch$yrbr$yr5F" "D" "stayIIv2" "agebin$yrepr avgres${varoi}rankage$yrepr stayIIv2_${yrbr}${yr5F}"
}
	 
foreach st in II{
	drop _all
	use Dres${varoi}ch$yrbr${yrer}stay`st'v2.dta 
	erase Dres${varoi}ch$yrbr${yrer}stay`st'v2.dta
	outsheet using Dres${varoi}ch$yrbr${yrer}stay`st'v2.txt, replace
}

if($yr6F>$yr5F & $yr5F>${yrer}){
	drop _all
	use Dres${varoi}ch$yrbr${yr5F}stayIIv2.dta 
	erase Dres${varoi}ch$yrbr${yr5F}stayIIv2.dta
	outsheet using Dres${varoi}ch$yrbr${yr5F}stayIIv2.txt, replace
}
