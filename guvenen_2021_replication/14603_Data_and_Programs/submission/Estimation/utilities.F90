!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!	MODULE UTILITIES HANDY SUBROUTINES, FUNCTIONS etc.
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
module UTILITIES
	IMPLICIT NONE
	...
	REAL(DP), PARAMETER :: truncate=2*10.0_DP**4	 ! Income values larger than this truncated.
	...
	END INTERFACE
	INTERFACE write2
		MODULE PROCEDURE write2_dp, write2_sp
	END INTERFACE
	INTERFACE write4
		MODULE PROCEDURE write4_dp, write4_sp
	END INTERFACE
	INTERFACE myread2
...
!BL
	...
	subroutine write_5(myarray5, filename5)
	IMPLICIT NONE

	!dummy arguments
	REAL(DP), DIMENSION(:,:,:,:,:), INTENT(IN):: myarray5
	...
	DO i=1,fileDIM(1)
		DO j=1,fileDIM(2)
			DO k=1,fileDIM(3)
				DO z=1,fileDIM(4)
					WRITE(11,100) (myarray5(i,j,k,z,m), m=1,fileDIM(5))
				END DO
			END DO
		END DO
	END DO
	...
	DO i=1,fileDIM(1)
	  DO j=1,fileDIM(2)
	   DO k=1,fileDIM(3)
			...
			WRITE(11,100) (myarray4(i,j,k,m), m=1,fileDIM(4))
		END DO
	   END DO
	END DO
	...
	CLOSE(UNIT=11)
	100 FORMAT(1x,1000(3x,F20.12))
	end subroutine  write4_DP

    subroutine write4_SP(myarray4, filename4)
	IMPLICIT NONE
	!dummy arguments
	REAL(SP), DIMENSION(:,:,:,:), INTENT(IN):: myarray4
	CHARACTER(LEN =*), INTENT(IN) :: filename4
	!local
	INTEGER, DIMENSION(4) :: fileDIM
	INTEGER:: i,j,k,m
	...
	CLOSE(UNIT=11)
	100 FORMAT(1x,1000(3x,F20.12))
	end subroutine  write4_SP


	subroutine write_3(myarray3, filename3)

	IMPLICIT NONE

	!dummy arguments
	REAL(DP), DIMENSION(:,:,:), INTENT(IN):: myarray3
	...
	DO i=1,fileDIM(1)
	  DO j=1,fileDIM(2)
			WRITE(11,100) (myarray3(i,j,k), k=1,fileDIM(3))
	   END DO
	END DO
	...
	subroutine write2_DP(myarray2, filename2)
		IMPLICIT NONE
		...
		DO i=1,fileDIM(1)
			WRITE(11,100) (myarray2(i,j), j=1,fileDIM(2))
		END DO
    ...
    end subroutine  write2_DP

    subroutine write2_SP(myarray2, filename2)
		IMPLICIT NONE

		!dummy arguments
		REAL(SP), DIMENSION(:,:), INTENT(IN):: myarray2
		CHARACTER(LEN =*), INTENT(IN) :: filename2
    ...
    end subroutine  write2_SP

	subroutine myread1_DP(mynum1, filename1)
		IMPLICIT NONE
		REAL(DP), INTENT(OUT):: mynum1
		CHARACTER(LEN =*), INTENT(IN) :: filename1
    INTEGER :: ios

	  ios=1
    do while(ios.Ne.0)
        open(32,file=filename1,STATUS='old')
        read(32,*,IOSTAT=ios) mynum1
        if(ios.Ne.0) print*,'myread1_DP(32,*,IOSTAT=ios)', ios
        close(32)
    enddo

	end subroutine  myread1_DP

	subroutine myread1_INT(mynum1, filename1)
		IMPLICIT NONE
		INTEGER, INTENT(OUT):: mynum1
		CHARACTER(LEN =*), INTENT(IN) :: filename1
		INTEGER :: ios
		...
		do while(ios.Ne.0)
				open(32,file=filename1,STATUS='old')
				read(32,*,IOSTAT=ios) mynum1
				if(ios.Ne.0) print*,'myread1_INT(32,*,IOSTAT=ios)', ios
				close(32)
		enddo
	...
	subroutine write_1(myarray1, filename1)
		IMPLICIT NONE

		!dummy arguments
		REAL(DP), DIMENSION(:), INTENT(IN):: myarray1
		CHARACTER(LEN =*), INTENT(IN) :: filename1
		...
		fileDIM= SHAPE(myarray1)
		DO i=1,fileDIM(1)
			WRITE(11,100) myarray1(i)
		END DO
		CLOSE(UNIT=11)
		100 FORMAT(1x,100(3x,F20.12))
	...
	SUBROUTINE WRITEMatrix(MATRIX, NR, NC,filename)
  		...
  		REAL(DP), DIMENSION(NR,NC) , INTENT(IN) :: MATRIX
		...
		DO ROW = 1, NR
			DO J = 1, NC
   	     		WRITE (11, '(F20.12," ")', ADVANCE = 'NO') MATRIX(ROW,J)
    				WRITE (11, '(",")', ADVANCE = 'NO')
			END DO
     		WRITE (11,"()")
  		END DO
...
!****************************************************************
	SUBROUTINE BILINEAR_INT(XX,YY,ZZ,x1,y1,z1)
		...
		IMPLICIT NONE
		REAL(DP), DIMENSION(:), INTENT(IN) :: XX, YY
		REAL(DP), DIMENSION(:,:), INTENT(IN) :: ZZ
...
!BL
	...
	SUBROUTINE variance(arr,var)
		REAL(DP), DIMENSION(:), INTENT(IN) :: arr
		REAL(DP), PARAMETER   :: missing= dpmissing-1.0_DP
		REAL(DP), INTENT(OUT) :: var
		REAL(DP) :: mean
		INTEGER  :: i,numobs

		numobs=count(arr<missing)
        if (numobs <= 1) call nrerror('moment: numobs must be at least 2')

		mean=0.0_DP
		do i=1,size(arr)
			if (arr(i)<missing) then
				mean=mean+arr(i)
			endif
		enddo
		mean=mean/dble(numobs)
		...
		enddo
		var=var/(dble(numobs)-1.0_DP)
	END SUBROUTINE variance

	SUBROUTINE mean_var(arr,mean,var)
		REAL(DP), DIMENSION(:), INTENT(IN) :: arr
		REAL(DP), PARAMETER   :: missing= dpmissing-1.0_DP
		REAL(DP), INTENT(OUT) :: var,mean

		INTEGER  :: i,numobs

		numobs=count(arr<missing)
        if (numobs <= 1) call nrerror('moment: numobs must be at least 2')
		mean=sum(arr,mask=(arr<missing))/dble(numobs)
		var=sum((arr-mean)*(arr-mean),mask=(arr<missing))/dble(numobs-1)
	END SUBROUTINE mean_var


	SUBROUTINE summarize(arr,numobs,mean,sdev,skew,kurt,p1,p5,p10,p25,p50,p75,p90,p95,p99)
		REAL(DP), DIMENSION(:), INTENT(IN) :: arr
		REAL(DP), DIMENSION(SIZE(arr)) :: sorted_arr
		REAL(DP), DIMENSION(:), ALLOCATABLE :: p,s
		REAL(DP), PARAMETER :: missing= dpmissing-1.0_DP
		REAL(DP), INTENT(OUT) :: mean,sdev,skew,kurt,p1,p5,p10,p25,p50,p75,p90,p95,p99
		...
		numobs=count(arr<missing)
		if (numobs <= 1) call nrerror('moment: numobs must be at least 2')
		allocate(p(numobs)); allocate(s(numobs))
		sorted_arr=arr
		call quicksort(sorted_arr)
		p=sorted_arr(1:numobs)
		mean=sum(p)
		mean=mean/dble(numobs)
		s(:)=p(:)-mean
		p(:)=s(:)*s(:)
		...
		sdev=dsqrt(var)
		if (var /= 0.0_DP) then
			skew=skew/(dble(numobs)*sdev**3)
			kurt=kurt/(dble(numobs)*var**2) !-3.0_DP This is the one used by STATA
		else
			call nrerror_continue('moment: no skewness or kurtosis when zero variance')
		end if
		deallocate(p); deallocate(s)

		i=floor(0.01*numobs)+1
		p1=sorted_arr(i)
		i=floor(0.05*numobs)+1
		p5=sorted_arr(i)
		i=floor(0.10*numobs)+1
		p10=sorted_arr(i)
		i=floor(0.25*numobs)+1
		p25=sorted_arr(i)
		i=floor(0.50*numobs)+1
		p50=sorted_arr(i)
		i=floor(0.75*numobs)+1
		p75=sorted_arr(i)
		i=floor(0.90*numobs)+1
		p90=sorted_arr(i)
		i=floor(0.95*numobs)+1
		p95=sorted_arr(i)
		i=min(numobs,floor(0.99*numobs)+1)
		p99=sorted_arr(i)
	END SUBROUTINE summarize

	SUBROUTINE summarize2(arr,numobs,mean,sdev,skew,kurt,p1,p5,p10,p12,p25,p37,p50, &
		p62,p75,p87,p90,p95,p99)
		REAL(DP), DIMENSION(:), INTENT(IN) :: arr
		REAL(DP), DIMENSION(SIZE(arr)) :: sorted_arr
		REAL(DP), DIMENSION(:), ALLOCATABLE :: p,s
		REAL(DP), PARAMETER :: missing= dpmissing-1.0_DP
		REAL(DP), INTENT(OUT) :: mean,sdev,skew,kurt,p1,p5,p10,p12,p25,p37,p50, &
			p62,p75,p87,p90,p95,p99
		INTEGER,  INTENT(OUT) :: numobs
		REAL(DP) :: var
		INTEGER :: i
		...
		i=floor(0.10*numobs)+1
		p10=sorted_arr(i)
		i=floor(0.125*numobs)+1
		p12=sorted_arr(i)
		i=floor(0.25*numobs)+1
		p25=sorted_arr(i)
		i=floor(0.375*numobs)+1
		p37=sorted_arr(i)
		i=floor(0.50*numobs)+1
		p50=sorted_arr(i)
		i=floor(0.625*numobs)+1
		p62=sorted_arr(i)
		i=floor(0.75*numobs)+1
		p75=sorted_arr(i)
		i=floor(0.875*numobs)+1
		p87=sorted_arr(i)
		i=floor(0.90*numobs)+1
		p90=sorted_arr(i)
		i=floor(0.95*numobs)+1
	...
	function covar(x,y)
		...
		REAL(DP), INTENT(IN) :: y(:)
		REAL(DP) :: covar
		REAL(DP) :: xy, sumx, sumy
		REAL(DP), PARAMETER :: missing= dpmissing-1.0_DP
		INTEGER :: numobs
		REAL(DP), DIMENSION(SIZE(x)) :: xnomiss,ynomiss

    numobs=COUNT(max(x,y)<missing)
    xnomiss=0.0_DP; ynomiss=0.0_DP
    WHERE(x<missing .and. y<missing)
       xnomiss=x
       ynomiss=y
    endwhere

		xy=DOT_PRODUCT(xnomiss,ynomiss)
		sumx=sum(xnomiss)
		sumy=sum(xnomiss)
		covar=xy/numobs-(sumx/numobs)*(sumy/numobs)

	end function covar

	SUBROUTINE SdSkewKurt(arr,sdev,skew,kurt,mask)
    REAL(DP), DIMENSION(:), INTENT(IN) :: arr
    REAL(DP), PARAMETER :: missing= dpmissing-1.0_DP
    REAL(DP), INTENT(OUT) :: sdev,skew,kurt
    LOGICAL(LGT), DIMENSION(:), INTENT(IN), OPTIONAL :: mask
    INTEGER   :: numobs
    REAL(DP)  :: mean,var
    REAL(DP), DIMENSION(SIZE(arr)) :: arrt,arrm,arrsd,arrsk,arrkt

    sdev=0.0_DP; skew=0.0_DP; kurt=0.0_DP
    ...
    IF(PRESENT(mask)) THEN
       ...
       endwhere
    ENDIF
    numobs=COUNT(arrt<missing)
    IF (numobs <= 1) CALL nrerror('moment: numobs must be at least 2')

    arrm=0.0_DP; arrsd=0.0_DP; arrsk=0.0_DP; arrkt=0.0_DP;
    WHERE(arrt<missing)
       arrm=arrt
    endwhere
    mean=SUM(arrm)/DBLE(numobs)
    WHERE(arrt<missing)
       arrsd=(arrt-mean)**2
       arrsk=(arrt-mean)**3
       arrkt=(arrt-mean)**4
    endwhere
    sdev=dsqrt(SUM(arrsd)/DBLE(numobs-1))
    IF (sdev /= 0.0_DP) THEN
       skew=SUM(arrsk)/(DBLE(numobs)*sdev**3)
       kurt=SUM(arrkt)/(DBLE(numobs)*sdev**4) !-3.0_DP This is the one used by STATA
    ELSE
       CALL nrerror_continue('moment: no skewness or kurtosis when zero variance')
    END IF
  END SUBROUTINE SdSkewKurt

    SUBROUTINE mypctile(arr,pctiles,pct_values,numobs)
        REAL(DP), DIMENSION(:), INTENT(IN) :: arr
        REAL(DP), PARAMETER :: pr_missing= dpmissing-1.0_DP
        REAL(DP), DIMENSION(size(arr)) :: sorted_arr
        REAL(DP), DIMENSION(:), INTENT(IN) :: pctiles
        REAL(DP), DIMENSION(size(pctiles)), INTENT(OUT) :: pct_values
        INTEGER, INTENT(OUT) :: numobs
        INTEGER :: i,j

        numobs=count(arr<pr_missing)
        if(numobs==0) call nrerror('no non-missing obs IN mypctile')
        sorted_arr=arr
        call quicksort(sorted_arr)
        do i=1,size(pctiles)
            j=min(numobs,floor(pctiles(i)*numobs/100)+1)
            pct_values(i)=sorted_arr(j)
        enddo
    END SUBROUTINE mypctile

		SUBROUTINE topshare(arr,pctiles,TopShr,numobs)
				REAL(DP), DIMENSION(:), INTENT(IN) :: arr
				REAL(DP), PARAMETER :: pr_missing= dpmissing-1.0_DP
				REAL(DP), DIMENSION(size(arr)) :: sorted_arr
				REAL(DP), DIMENSION(:), INTENT(IN) :: pctiles
				REAL(DP), DIMENSION(size(pctiles)), INTENT(OUT) :: TopShr
				INTEGER, INTENT(OUT) :: numobs
				REAL(DP) :: total
				INTEGER :: i,j,lb
				...
				sorted_arr=arr
				call quicksort(sorted_arr)
				total=sum(sorted_arr(1:numobs))
				do i=1,size(pctiles)
					lb=FLOOR(DBLE(numobs*pctiles(i)/100))+1
					TopShr(i)=sum(sorted_arr(lb:numobs))/total
				enddo
		END SUBROUTINE topshare

    SUBROUTINE smthpctile(arr,pctiles,pct_values,numobs)
        REAL(DP), DIMENSION(:), INTENT(IN) :: arr
        REAL(DP), PARAMETER :: pr_missing = dpmissing-1.0_DP
        REAL(DP), DIMENSION(size(arr)) :: sorted_arr
        REAL(DP), DIMENSION(:), INTENT(IN) :: pctiles
        ...
        INTEGER, INTENT(OUT) :: numobs
        INTEGER :: i,j,jl,jh
        INTEGER, PARAMETER :: bin=5

        numobs=count(arr<pr_missing)
        if(numobs==0) call nrerror('no non-missing obs IN smthpctile')
        sorted_arr=arr
        call quicksort(sorted_arr)
        do i=1,size(pctiles)
            j=min(numobs,floor(pctiles(i)*numobs/100)+1)
            jl=max(1,j-bin)
            jh=min(j+bin,numobs)
            pct_values(i)=sum(sorted_arr(jl:jh))/(jh-jl+1)
        enddo
    END SUBROUTINE smthpctile

    FUNCTION sortrows(matt,col,numobs) ! This subroutine sorts rows of matt by its 1st column
        IMPLICIT NONE
	...
	subroutine Histogram(arr,nbin,filename,cutoff_input)
		implicit none
		REAL(8), dimension(:), intent(in) :: arr    ! arr(nsim,Hmax)
		INTEGER, INTENT(IN) :: nbin
		CHARACTER(LEN =*), INTENT(IN) :: filename
		REAL(8), OPTIONAL, INTENT(IN) :: cutoff_input
		REAL(8)	 :: hist(nbin+1,2),bin(nbin+1),dx,cutoff=0.01_DP 	!
    REAL(DP), PARAMETER :: missing= dpmissing-1.0_DP
		REAL(8), DIMENSION(size(arr)) :: sorted_arr
		real(DP), allocatable :: p(:)
		integer :: numobs,i,k,ir

		cutoff=0.01_DP
		if(PRESENT(cutoff_input)) cutoff=cutoff_input

		sorted_arr=arr
		numobs=count(sorted_arr<missing)
		if (numobs <= 1) call nrerror('Histogram: numobs must be at least 2')
		call quicksort(sorted_arr)
		bin(1)=sorted_arr(floor(cutoff*numobs/100.0_DP)+1)
		bin(nbin)=sorted_arr(floor((100_DP-cutoff)*numobs/100.0_DP)+1)
		bin(nbin+1)=sorted_arr(numobs)+1.0_DP
		dx=(bin(nbin)-bin(1))/dble(nbin-1)
		hist(1,1)=bin(1)-dx
		do i=1,nbin-2
			bin(i+1)=bin(1)+dx*i
			hist(i+1,1)=hist(1,1)+dx*i
		enddo
		hist(nbin,1)=bin(nbin)-dx
		hist(nbin+1,1)=bin(nbin)+dx
		hist(:,2)=0.0_DP
		k=1
		do i=1,numobs
			if(sorted_arr(i)<=bin(k)) then
				hist(k,2)=hist(k,2)+1.0_DP
			else
				k=k+1
				hist(k,2)=hist(k,2)+1.0_DP
			endif
		enddo
		hist(:,2)=hist(:,2)/numobs
		call write2(hist,filename)
   ...
   FUNCTION CHOL(AIN) ! Cholesky Decomposition
    IMPLICIT NONE
    REAL(DP), DIMENSION(:,:), INTENT(IN) :: AIN
    REAL(DP) :: A(SIZE(AIN,1),SIZE(AIN,2)),p(SIZE(AIN,1)),CHOL(SIZE(AIN,1),SIZE(AIN,2))
    INTEGER :: i,j,n
    REAL(DP) :: summ
...
MODULE ran_state
    USE UTILITIES, ONLY : nrerror, DP, SP, LGT, I4B
	IMPLICIT NONE
