 Module DFNLS
  ...
  USE OBJECTIVE, ONLY : nmoments, nest
  REAL(DP) :: eps=epsilon(1.0_DP)

  INTEGER, PARAMETER :: SET_NMAX=nest+1 ! It should be bigger than the lengh of the vector function v_err(x).
  INTEGER, PARAMETER :: SET_MMAX=nmoments+2 ! It should be bigger than the lengh of the vector function v_err(x).
...
!
      ...
      PARAMETER (nmax = SET_NMAX, mmax=SET_MMAX, nptmax=2*nmax+1)
...
!   BOBYQA, authored by M. J. D. Powell.
      ...
      IMPLICIT double precision (A-H,O-Z)
      PARAMETER (nmax = SET_NMAX, mmax=SET_MMAX)
