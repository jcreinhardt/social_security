...
folder=char('13May2014JobChange');
% folder2=char('1_June_2016_STAY_LABOR_EIN');
if(user==1) % serdar
	userdirectory=char('C:\Research\SSA-INCOME-RISK\EmpiricalResults\CrossSectional\');
	sep=char('\');
elseif(user==2) % f. karahan nyfed
	userdirectory=char('C:\Users\rceyfk01\Dropbox\SSA-INCOME-RISK\EmpiricalResults\CrossSectional\');
	sep=char('\');
	...
	addpath('C:\Users\rceyfk01\Dropbox\Matlab\line_fewer_markers');
...
output = [userdirectory 'Output' sep 'Stayers' sep 'AltDef'];
filename = 'cross_moments_stay_switch_altdef.xlsx';
...
for t=tbegin:tlast
	...
	K=importdata(['Dreswagech' num2str(t) num2str(t+k) 'stayIv2.txt']);
	...
	p1(:,t-tbegin+1)=PRarr(t-tbegin+1,:,11);
	p5(:,t-tbegin+1)=PRarr(t-tbegin+1,:,12);
	p10(:,t-tbegin+1)=PRarr(t-tbegin+1,:,13);
	p25(:,t-tbegin+1)=PRarr(t-tbegin+1,:,14);
	p50(:,t-tbegin+1)=PRarr(t-tbegin+1,:,15);
	p75(:,t-tbegin+1)=PRarr(t-tbegin+1,:,16);
	p90(:,t-tbegin+1)=PRarr(t-tbegin+1,:,17);
	p95(:,t-tbegin+1)=PRarr(t-tbegin+1,:,18);
	p99(:,t-tbegin+1)=PRarr(t-tbegin+1,:,19);

	K2=importdata(['Dreswagech' num2str(t) num2str(t+k) 'stayIIv2.txt']);
	PRarr2(t-tbegin+1,:,:)=K2.data;
	n_v2(:,t-tbegin+1)=PRarr2(t-tbegin+1,:,4);
	sd_v2(:,t-tbegin+1)=PRarr2(t-tbegin+1,:,6);
	skew_v2(:,t-tbegin+1)=PRarr2(t-tbegin+1,:,7);
	kurt_v2(:,t-tbegin+1)=PRarr2(t-tbegin+1,:,8);
	p1_v2(:,t-tbegin+1)=PRarr2(t-tbegin+1,:,11);
	p5_v2(:,t-tbegin+1)=PRarr2(t-tbegin+1,:,12);
	p10_v2(:,t-tbegin+1)=PRarr2(t-tbegin+1,:,13);
	p25_v2(:,t-tbegin+1)=PRarr2(t-tbegin+1,:,14);
	p50_v2(:,t-tbegin+1)=PRarr2(t-tbegin+1,:,15);
	p75_v2(:,t-tbegin+1)=PRarr2(t-tbegin+1,:,16);
	p90_v2(:,t-tbegin+1)=PRarr2(t-tbegin+1,:,17);
	p95_v2(:,t-tbegin+1)=PRarr2(t-tbegin+1,:,18);
	p99_v2(:,t-tbegin+1)=PRarr2(t-tbegin+1,:,19);
end
kellys=((p90-p50)-(p50-p10))./(p90-p10);
hinkley=((p99-p50)-(p50-p1))./(p99-p1);
p90_10=p90-p10;

kellys_v2=((p90_v2-p50_v2)-(p50_v2-p10_v2))./(p90_v2-p10_v2);
hinkley_v2=((p99_v2-p50_v2)-(p50_v2-p1_v2))./(p99_v2-p1_v2);
p90_10_v2=p90_v2-p10_v2;
...
% 	crowsage(age,:)=mean(crows((age-1)*200+1:age*200,:),2)';
	...
	nage_v2(age,:)=mean(n_v2((age-1)*200+1:age*200,:),2)';
	sdage_v2(age,:)=mean(sd_v2((age-1)*200+1:age*200,:),2)';
	p90_10age_v2(age,:)=mean(p90_10((age-1)*200+1:age*200,:),2)';
	skewage_v2(age,:)=mean(skew_v2((age-1)*200+1:age*200,:),2)';
	kellyage_v2(age,:)=mean(kellys_v2((age-1)*200+1:age*200,:),2)';
	kurtage_v2(age,:)=mean(kurt_v2((age-1)*200+1:age*200,:),2)';
	hinkleyage_v2(age,:)=mean(hinkley_v2((age-1)*200+1:age*200,:),2)';
% 	moorsage_v2(age,:)=mean(moors_v2((age-1)*200+1:age*200,:),2)';
% 	crowsage_v2(age,:)=mean(crows_v2((age-1)*200+1:age*200,:),2)';
	p1age_v2(age,:)=mean(p1_v2((age-1)*200+1:age*200,:),2)';
	p5age_v2(age,:)=mean(p5_v2((age-1)*200+1:age*200,:),2)';
	p10age_v2(age,:)=mean(p10_v2((age-1)*200+1:age*200,:),2)';
	p25age_v2(age,:)=mean(p25_v2((age-1)*200+1:age*200,:),2)';
	p50age_v2(age,:)=mean(p50_v2((age-1)*200+1:age*200,:),2)';
	p75age_v2(age,:)=mean(p75_v2((age-1)*200+1:age*200,:),2)';
	p90age_v2(age,:)=mean(p90_v2((age-1)*200+1:age*200,:),2)';
	p95age_v2(age,:)=mean(p95_v2((age-1)*200+1:age*200,:),2)';
	p99age_v2(age,:)=mean(p99_v2((age-1)*200+1:age*200,:),2)';
end
cd(output);
...
for i=1:3
	...
	frac_stay_v2(i,:)=nage_v2(i,2:2:200)./(nage_v2(i,1:2:200)+nage_v2(i,2:2:200));
end
h=figure;
...
% if k==1
% 	x1=0; x2=100; y1=0.80; y2=1.00; st=0.05;
% else
% 	x1=0; x2=100; y1=0.80; y2=1.00; st=0.95;
% end
